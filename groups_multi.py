...
def get_hist():
    ...
    vol = logret.std()*np.sqrt(len(logret))
    return px_hist, cumret, logret, vol
...
def get_rol():
    ...
    rol_wgt = pd.DataFrame(norm.cdf(rol_rar), columns=rol_rar.columns, index=rol_rar.index)
    rol_wgt_score = pd.cut(rol_wgt.unstack(), [0, 0.2, 0.4, 0.6, 0.8, 1], labels=False).unstack().T + 1
    return rol_ret, rol_vol, rol_rar, rol_rar_avg, rol_wgt, rol_wgt_score
...
def plot_opt():
    ...
    fig_opt = make_subplots(rows=1, cols=3, shared_xaxes=True, shared_yaxes=True,
    ...
    fig_opt.add_trace(go.Scatter(x=stock_hist.index, y=stock_hist[ticker],
                             name=ticker+' close price', connectgaps=True, line={'color': snsblue, 'width': 2.5}, opacity=1),
                  row=1, col=1)

    fig_opt.add_trace(go.Scatter(x=[tx_date, ref_date], y=[ref_price_tx_date, ref_price], mode='markers',
                             showlegend=False, line={'color': snsblue, 'width': 2.5}, opacity=1),
                  row=1, col=1)

    fig_opt.add_trace(go.Scatter(x=stock_hist.index, y=stock_hist[ticker].rolling(td2e).mean(),
                             name=str(td2e)+' td SMA',line={'color':snsgrey,'width':1}),
                  row=1, col=1)

    fig_opt.add_trace(go.Scatter(x=opt_hist[opt_hist['cd2e'] <= (cd2e + 20)].index,
                             y=opt_hist[opt_hist['cd2e'] <= (cd2e + 20)]['breakeven'],
                             name='BS approx', connectgaps=True, mode='lines', line={'color': snsorange, 'width': 2.5},
                             opacity=0.4),
                  row=1, col=1)

    fig_opt.add_trace(go.Scatter(x=arrow.index, y=px_hist[tkr].shift(win).iloc[-1] * (1 + arrow[tkr] * rol_vol[tkr].iloc[-1]),
                                 mode='lines',  line=dict(color=quad['Color'][tkr], width=5), opacity=0.5,
                                 name='Close Price'),
                  row=1, col=1)

    fig_opt.add_trace(go.Scatter(x=bell, y=range, name='norm dist', mode='lines',
                             line={'color': snsblue, 'width': 0.75}, fill='tozerox'),
                  row=1, col=3)

    fig_opt.add_trace(go.Scatter(x=[max(bell)], y=[ref_price_tx_date], mode='markers',
                             showlegend=False, line={'color': snsblue, 'width': 2.5}, opacity=1),
                  row=1, col=3)
    ...
    for l, p in zip(levels_short, pnl_short):
        ...
        fig_opt.add_trace(go.Scatter(x=[tx_date, exp_date], y=[l, l], mode='lines+text', opacity=o,
        ...
        fig_opt.add_trace(go.Scatter(x=[min(pnl), max(pnl)], y=[l, l], mode='lines+text', name='',
        ...
        # fig_opt.add_trace(go.Scatter(x=[stock_hist.index.min(), exp_date],
        #                          y=[l / stock_hist[ticker].iloc[-1] - 1, l / stock_hist[ticker].iloc[-1] - 1],
        #                          showlegend=False, mode='lines', opacity=o,
        #                          line={'color': color, 'width': width, 'dash': 'dashdot'}),
        #               row=2, col=1)

        fig_opt.add_trace(go.Scatter(x=[0, max(bell)*1.25], y=[l, l],
    ...
    fig_opt.add_trace(go.Scatter(x=pnl, y=levels, name='payoff diagram',
    ...
    fig_opt.add_trace(go.Scatter(x=[min(pnl), max(pnl)+padding],
                             y=[levels[0], levels[0]],
                             name='', opacity=0.0, showlegend=False),
                  row=1, col=2)

    fig_opt.add_trace(go.Scatter(x=[tx_date, ref_date], y=[level_tx, level_last],
                             showlegend=False, mode='markers', marker=dict(color=snsorange), opacity=1),
                  row=1, col=1)

    fig_opt.add_trace(go.Scatter(x=[0, pnl_last], y=[level_tx, level_last],
                             showlegend=False, mode='lines+markers', marker=dict(color=snsorange), opacity=1),
                  row=1, col=2)

    if strategy in ['Straddle', 'Strangle']:
        fig_opt.add_trace(go.Scatter(x=[tx_date, ref_date], y=[put_strikes[0]-tx_price, put_strikes[0]-lastPrice],
                                 showlegend=False, mode='markers', marker=dict(color=snsorange), opacity=1),
                      row=1, col=1)
        fig_opt.add_trace(go.Scatter(x=[0, pnl_last], y=[put_strikes[0]-tx_price, put_strikes[0]-lastPrice],
                                 showlegend=False, mode='lines+markers', marker=dict(color=snsorange), opacity=1),
                      row=1, col=2)
        fig_opt.add_trace(go.Scatter(x=opt_hist[opt_hist['cd2e'] <= (cd2e + 20)].index,
    ...
    # fig_opt.add_trace(go.Scatter(x=rol_vol.index, y=rol_vol,
    ...
    #               row=2, col=1)
    #
    # fig_opt.add_trace(go.Scatter(x=[tx_date,tx_date], y=[solver_vol*np.sqrt(td2e/252), -solver_vol*np.sqrt(td2e/252)],
    #                          name='ivol solver', mode='markers',
    #                          line={'color': snsorange, 'width': 1.25}),
    #               row=2, col=1)


    fig_opt.update_xaxes(zerolinecolor='grey', zerolinewidth=1.25, showticklabels=False, col=2, row=1)
    fig_opt.update_xaxes(showticklabels=False, col=3, row=1)
    fig_opt.update_yaxes(zerolinecolor='grey', zerolinewidth=1.25, tickformat='%', col=1, row=2)
    fig_opt.update_layout(margin=dict(l=0, r=0, t=50, b=0), template='seaborn', plot_bgcolor='#F0F2F6')
    fig_opt.update_layout(height=620, width=1200)  #, paper_bgcolor='yellow')
    fig_opt.update_layout(legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01))
...
def plot_scatter():
    ...
    for t in cumret.columns:
        fig_scatter.add_trace(go.Scatter(x=[vol.loc[t]], y=cumret[t].tail(1),
                                         mode='markers', line=dict(color=snsgrey, width=3),
                                         name=t, opacity=0.8),
                              row=1, col=3)
    ...
    # fig_scatter.add_trace(go.Scatter(x=arrow.index, y=px_hist[tkr].shift(win).iloc[-1] * (1 + arrow[tkr] * rol_vol[tkr].iloc[-1]),
    #                              mode='lines',  line=dict(color=quad['Color'][tkr], width=5), opacity=0.5,
    #                              name='Close Price'),
    #                   row=1, col=1)
    ...
    # fig_scatter.add_trace(go.Scatter(x=px_hist.index, y=px_hist[tkr].shift(win)*(1+spline[tkr]*rol_vol[tkr].iloc[-1]),
    #                              mode='lines', line=dict(color='red', width=3),
    #                              name='Spline'),
    #                   row=1, col=1)
    ...
    fig_scatter.add_trace(go.Scatter(x=spline.tail(tail).index, y=spline[tkr].tail(tail),
                                    mode='lines', line=dict(color=quad['Color'][tkr], width=4),
                                    name='TC Momentum'),
                      row=3, col=1)

    fig_scatter.add_trace(go.Scatter(x=arrow.index, y=arrow[tkr],
                                    mode='lines', line=dict(color=quad['Color'][tkr], width=5), opacity=0.5,
                                    name='TC Momentum'),
                      row=3, col=1)

    fig_scatter.add_trace(go.Scatter(x=spline.tail(1).index, y=spline[tkr].tail(1),
                                    mode='markers', line=dict(color=quad['Color'][tkr], width=3),
                                    marker=dict(color='white', size=10,
                                             line=dict(color=quad['Color'][tkr], width=2)),
                                    name='TC Momentum'),
    ...
    for l, c, o, w in zip(level, color, opacity, width):
        fig_scatter.add_hline(y=l, line_color=c, opacity=o, line_width=w, line_dash="dash", row=3, col=1)
        fig_scatter.add_trace(go.Scatter(x=[0, vol.loc[tkr]], y=[0, vol.loc[tkr]*l], mode='lines',
                                         line_color=c, opacity=o, line_width=w, line_dash="dash"),
                              row=1, col=3)

    fig_scatter.update_layout(showlegend=False, margin=dict(l=0, r=10, t=50, b=30), plot_bgcolor='#f0f2f6',
                          width=1400, height=650)    # paper_bgcolor = 'lightyellow'

    fig_scatter.update_xaxes(zerolinecolor='white', zerolinewidth=3)
    fig_scatter.update_xaxes(showticklabels=False, row=1, col=1)
    fig_scatter.update_xaxes(showticklabels=False, row=1, col=2)
    fig_scatter.update_xaxes(autorange='reversed', showticklabels=False, row=1, col=3)
    fig_scatter.update_xaxes(matches='x3', showticklabels=True, tickformat=',.1%', row=2, col=3)
    fig_scatter.update_xaxes(matches='x', showticklabels=True, row=3, col=1)


    fig_scatter.update_yaxes(zerolinecolor='white', zerolinewidth=3)
    fig_scatter.update_yaxes(title_text="Cumulative Return", tickformat=',.1%', row=1, col=1)
    fig_scatter.update_yaxes(matches='y', showticklabels=False, row=1, col=2)
    fig_scatter.update_yaxes(matches='y', showticklabels=False, row=1, col=3)
    fig_scatter.update_yaxes(range=[-5,5], row=3, col=1)
...
px_hist, cumret, logret, vol = get_hist()
rol_ret, rol_vol, rol_rar, rol_rar_avg, rol_wgt, rol_wgt_score = get_rol()
...
rec['tile'] = norm.cdf(rec['Improvement'])
rec['abs_tile'] = norm.cdf(rec['Score'])
...
if opt == 'Scatter':
    ...
    with col22:
        ...
        ptf = pd.concat([rol_rar.tail(1).T,
                         ...
                         pred],
                        axis=1, keys=['rar', 'p', 'last score', 'predict'])
        ptf.columns = ptf.columns.droplevel(level=1)
        ptf = ptf.dropna()
        ptf['updown'] = ptf['predict'] - ptf['last score']
        df_actions =  pd.DataFrame({'updown': [*range(-4,5)],
                                    'action': ['max reversal','reversal','dbl downgrade','downgrade','unch',
